...
function merge(local,remote){if(!remote)return local;const session=Number(local.session?.updatedAt||0)>=Number(remote.session?.updatedAt||0)?local.session:remote.session;const settings=Object.keys(local.settings||{}).length?local.settings:(remote.settings||{});return{schemaVersion:1,state:mergeState(local.state,remote.state),settings,session:session||null,updatedAtClient:Date.now()}}
...
async function signIn(){const provider=new firebase.auth.GoogleAuthProvider();provider.setCustomParameters({prompt:'select_account'});message('Googleログインを開いています…','working');try{await auth.signInWithPopup(provider)}catch(error){if(['auth/popup-blocked','auth/operation-not-supported-in-this-environment'].includes(error?.code)){await auth.signInWithRedirect(provider);return}console.error(error);message('ログインに失敗','error')}}
